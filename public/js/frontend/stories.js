if (typeof window.StoryManager === "undefined") {
    window.StoryManager = class StoryManager {
        constructor() {
            this.data = [];
            this.userIndex = 0;
            this.slideIndex = 0;
        ...
        _renderSlide(index, immediate = false) {
            ...
            requestAnimationFrame(() =>
                requestAnimationFrame(() => this._activateSlide(index)),
            );
        }
        ...
        _activateSlide(index) {
            ...
            const isActive = () =>
                this.userIndex === capturedUser &&
                this.slideIndex === capturedSlide;
        ...
        _resume() {
            ...
            const content =
                this.data[this.userIndex]?.contents[this.slideIndex];
            if (content?.type === "video") {
                const slide = document.getElementById(
                    `${this._ns}-slide-${this.slideIndex}`,
                );
                slide
                    ?.querySelector("video")
                    ?.play()
        ...
        _updateNavButtons() {
            ...
            if (prev)
                prev.classList.toggle("story-nav-hidden", this.userIndex === 0);
            if (next)
                next.classList.toggle(
                    "story-nav-hidden",
                    this.userIndex >= this.data.length - 1,
                );
        }
    };
    ...
    function initStoryManager() {
        ...
        window.storyManager = new window.StoryManager();
    }
